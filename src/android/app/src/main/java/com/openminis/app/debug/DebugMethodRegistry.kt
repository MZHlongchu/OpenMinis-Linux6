...
object DebugMethodRegistry {
    ...
    private val BASE_METHODS: List<MethodSpec> = listOf(
        ...
        MethodSpec(
            ...
            description = "Export a provider instance (config + models + stored API key) as a portable JSON envelope. Mirrors the in-app Share/Export button. The API key is base64-wrapped to survive JSON string escaping; this is NOT a security boundary — callers of the 6321 RPC are trusted.",
            params = listOf(
                ParamSpec("instanceId", "string", required = true, description = "Target instance UUID."),
            ),
