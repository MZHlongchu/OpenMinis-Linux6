...
启动器名称是 **Minis Ultra**，`applicationId` 为 `com.openminis.linux`。当前版本 **1.36.4-linux**（versionCode 57）。
...
正式发行包：[Releases `1.36.4-linux`](https://github.com/tall-1997/OpenMinis-Linux/releases/tag/1.36.4-linux)。1.36.4：聊天里生成视频（Videos API / `generate_video`）。详见 [docs/RELEASE-NOTES.zh.md](docs/RELEASE-NOTES.zh.md)。
