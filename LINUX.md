...
启动器名称是 **Minis Ultra**，`applicationId` 为 `com.openminis.linux`。当前版本 **1.36.2-linux**（versionCode 55）。
...
正式发行包：[Releases `1.36.2-linux`](https://github.com/tall-1997/OpenMinis-Linux/releases/tag/1.36.2-linux)。1.36.2：中转 429 按模型桶处理；只有选中设置里的模型组才会按组序换人。详见 [docs/RELEASE-NOTES.zh.md](docs/RELEASE-NOTES.zh.md)。
