...
正式发行包：[Releases `1.20-linux`](https://github.com/tall-1997/OpenMinis-Linux/releases/tag/1.20-linux)。1.20：`search_sessions`/`read_session`、子 Agent `kind`/`write_paths`、系统助手入口与桌面小组件、browser_use 跟系统 WebView（目标 Chrome/151）、libunwind。1.19：主机事件通道、动态 minis-notify、任务级能力路由、沙箱长任务 FGS、http_proxy 日志/切断、WebDAV 恢复向导、aarch64 一键编译。详见 [docs/RELEASE-NOTES.zh.md](docs/RELEASE-NOTES.zh.md)。
