...
启动器名称是 **Minis Ultra**，`applicationId` 为 `com.openminis.linux`。当前版本 **1.36.12-linux**（versionCode 65）。
...
正式发行包：[Releases `1.36.12-linux`](https://github.com/tall-1997/OpenMinis-Linux/releases/tag/1.36.12-linux)。1.36.12：检查更新在缺安装权限或进程被杀后不再卡死、不再重下已下完的 APK。详见 [docs/RELEASE-NOTES.zh.md](docs/RELEASE-NOTES.zh.md)。
