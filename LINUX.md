...
启动器名称是 **Minis Ultra**，`applicationId` 为 `com.openminis.linux`。当前版本 **1.36.3-linux**（versionCode 56）。
...
正式发行包：[Releases `1.36.3-linux`](https://github.com/tall-1997/OpenMinis-Linux/releases/tag/1.36.3-linux)。1.36.3：热修发消息 `Flow invariant is violated` 崩溃。详见 [docs/RELEASE-NOTES.zh.md](docs/RELEASE-NOTES.zh.md)。
